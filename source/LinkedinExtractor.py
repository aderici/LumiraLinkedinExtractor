...
def parseArgs(fieldValues):
    ...
    for i in range(len(paramslist)):
        if paramslist[i].split('=')[0].lower() == 'consumer_key':
            try:
                fieldValues[0] = dpapi.dpapi_decrypt_data(paramslist[i].split('=')[1].decode('hex'), entropy=extra_entropy)
            except:
                fieldValues[0] = 'ENTER_CONSUMER_KEY'
        elif paramslist[i].split('=')[0].lower() == 'consumer_secret':
            try:
                fieldValues[1] = dpapi.dpapi_decrypt_data(paramslist[i].split('=')[1].decode('hex'), entropy=extra_entropy)
            except:
            ...
            try:
                ...
                fieldValues[1] = 'ENTER_USER_TOKEN'
        elif paramslist[i].split('=')[0].lower() == 'user_secret':
            try:
                fieldValues[1] = dpapi.dpapi_decrypt_data(paramslist[i].split('=')[1].decode('hex'), entropy=extra_entropy)
            except:
                fieldValues[1] = 'ENTER_USER_SECRET'
        i += 1
...
def printData(fieldValues):
    ...
    csv_date_format;d.M.yyyy;true"""
        print ''.join(['consumer_key;', dpapi.dpapi_encrypt_data(fieldValues[0], entropy=extra_entropy).encode('hex'), ';true'])
        print ''.join(['consumer_secret;', dpapi.dpapi_encrypt_data(fieldValues[1], entropy=extra_entropy).encode('hex'), ';true'])
        print ''.join(['user_token;', dpapi.dpapi_encrypt_data(fieldValues[2], entropy=extra_entropy).encode('hex'), ';true'])
        print ''.join(['user_secret;', dpapi.dpapi_encrypt_data(fieldValues[3], entropy=extra_entropy).encode('hex'), ';true'])
        print "endDSInfo"
        print "beginData"
